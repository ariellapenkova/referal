...
async def new_ref(id, ref):
    ...
    if ref_2[2] != 0:
        ...
        try:
            ...
            await update_balance(ref_2[2], 2.0)
        except:
            pass
        ...
        if ref_3[2] != 0:
            ...
            try:
                ...
                await update_balance(ref_3[2], 4.0)
            except:
                pass
...
stats.add(a, b, c)
stats.add(InlineKeyboardButton(text='Владелец 🧨', url='https://t.me/kredo_admin'))
...
<code>     </code><b>💸Выведено:</b> <code>{withdraws} ₽</code>
➖➖➖➖➖➖➖➖➖➖➖➖➖
...
1️⃣ Вы получите 1 с каждого рефералла на 1 уровне

2️⃣ Вы получите 2 с каждого рефералла на 2 уровне

3️⃣ Вы получите 4 с каждого рефералла на 3 уровне

💸 Ваша реф ссылка, по которой должен перейти рефералл:</b> https://t.me/telmanybot?start={uid}"""
...
1️⃣ Вы получите 4₽ с каждого рефералла на 1 уровне

2️⃣ Вы получите 2₽ ₽ с каждого рефералла на 2 уровне

3️⃣ Вы получите 1 ₽ с каждого рефералла на 3 уровне
...
a = InlineKeyboardButton(text='💸 Канал', url='https://t.me/kredo_official')
b = InlineKeyboardButton(text='🧑‍💻 Владелец', url='https://t.me/kredo_admin')
c = InlineKeyboardButton(text='🧃 Чат', url='https://t.me/joinchat/WPFr1M6OFjaDY-hb')
...
async def msg(m: Message, state: FSMContext):
    ...
    text = """<b>🚀 Приветствуем Вас в сервисе @Telmanybot
💰 Мы являемся лучшим ботом для заработка в Telegram!
▪️▪️Наш проект был запущен: 09.12.2021
 У нас 3-ёх уровневая система рефералов
💸 Мы платим:
➖➖➖➖➖➖➖➖➖➖
1️⃣ Вы получите 1₽ с каждого рефералла на 1 уровне
2️⃣ Вы получите 2₽ ₽ с каждого рефералла на 2 уровне
3️⃣ Вы получите 4 ₽ с каждого рефералла на 3 уровне
➖➖➖➖➖➖➖➖➖➖
🍀 Увеличь свой заработок без лишних усилий, и сделай себе независимым от мамки💰
▪️▪️Администрация бота: @kredo_admin
▪️▪️</b>"""
...
async def msg(c: CallbackQuery):
    ...
    if user[1] < 340:
        return await c.answer('На балансе должно быть не менее 340₽ ❌', show_alert=True)
    else:
...
async def aye(m: Message, state: FSMContext):
    ...
    if summ < 340:
        await state.finish()
        return await m.answer(reply_markup=main_kb, text=f'<b>🥃 Минимальная сумма вывода 340₽ !</b>')

    if user[1] < summ:
        await state.finish()
        return await m.answer(reply_markup=main_kb, text=f'<b>❌ На балансе недостаточно денег, ваш баланс:</b> <code>{round(user[1], 2)} ₽</code>')

    await m.answer(f'<b>🥝 Введите номер вашего QIWI кошелька</b> (<code>Пример: +7915555555</code>):\n\n(Если хотите вывести на BTC banker, то напишите ваш ник в телеграмм через @, с вами свяжется администратор)')
...
async def msg(c: CallbackQuery):
    ...
    await log_chat(text)
    text = f"""<b>💰 Вам было успешно выведено {summ} ₽
...
async def msg(c: CallbackQuery):
    ...
    text = f"""<b>💰 Вам успешно были возвращены средства, вывод не удался, сумма: {summ} ₽</b>"""
...
async def bonus(m: types.Message):
    ...
    if time.time() - user[6] >= 60*60*24:
        await update_balance(m.from_user.id, 4, True)
        await m.reply('Вы получили бонус 4₽ 🎁')
...
async def ayehshshsg(m: types.Message):
    ...
    await m.answer(f'<a href="tg://user?id={uid}">Пользователю</a> успешно выдано {summ} ₽')
...
async def ayehshshsg(m: types.Message):
    ...
    await m.answer(f'<a href="tg://user?id={uid}">Пользователю</a> успешно выдано на выводы {summ} ₽')
